...
const RecipeCard: React.FC<{ recipe: Recipe; onSelect: (recipe: Recipe) => void }> = ({ recipe, onSelect }) => {
    ...
    return (
        <div className="bg-white p-4 rounded-lg border border-neutral-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer" onClick={() => onSelect(recipe)}>
            <h3 className="font-semibold text-secondary">{recipe.name}</h3>
            <p className="text-sm text-neutral-600">{recipe.category}</p>
            <div className="mt-4 flex justify-between items-baseline">
                <div>
                    <p className="text-xs text-neutral-500">Cost/Portion</p>
                    <p className="font-bold text-lg text-primary">${recipe.costPerPortion.toFixed(2)}</p>
                </div>
                <div>
                    <p className="text-xs text-neutral-500 text-right">Target Margin</p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl m-4">
                <div className="p-6 border-b">
                    <h2 className="text-xl font-bold text-secondary">{recipe.name}</h2>
                    <p className="text-sm text-neutral-600">{recipe.category}</p>
                </div>
                ...
                <div className="p-6 bg-neutral-50 rounded-b-lg flex justify-end space-x-4">
